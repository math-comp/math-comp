...
Lemma leq_leqP {m n} : reflect (forall k, n <= k -> m <= k) (m <= n).
...
Lemma eqn_geqP {m n} : reflect (forall k, (k <= m) = (k <= n)) (m == n).
Proof.
apply: (iffP idP) => [/eqP->//|/[dup]].
by rewrite eqn_leq => -> <-; rewrite !leqnn.
Qed.

Lemma eqn_leqP {m n} : reflect (forall k, (m <= k) = (n <= k)) (m == n).
Proof.
apply: (iffP idP) => [/eqP->//|/[dup]].
by rewrite eqn_leq => <- ->; rewrite !leqnn.
Qed.
