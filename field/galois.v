...
#[warnings="-deprecated", deprecated(since="mathcomp 2.5.0",
      note="use `kHom'P` instead")]
Lemma kHomP {K V f} :
  reflect [/\ {in V &, forall x y, f (x * y) = f x * f y} &
           {in K, forall x, f x = x}]
          (kHom K V f).
Proof. by apply: (iffP kHom'P) => [][]. Qed.
...
Proof.
by case/kHom'P=> idKf _ /polyOverP Kp; apply/polyP=> i; rewrite coef_map /= idKf.
Qed.
...
Proof. by move/subvP=> sV12 /kHom'P[idKf /(sub_in2 sV12)fM]; apply/kHom'P. Qed.
...
Proof.
move=> homKf; have [idKf fM] := kHom'P homKf.
...
Proof.
have [idKf fM] := kHom'P homKf.
apply/kHom'P; split=> [z Kz|]; first by rewrite kHomExtend_id ?(subvP sKE) ?idKf.
...
Proof.
move=> homKf; have [[idKf fM] kerf0] := (kHom'P homKf, kAutf_lker0 homKf).
have f1K: cancel f^-1%VF f by apply: lker0_lfunVK.
apply/kHom'P; split=> [x Kx | x y _ _]; apply: (lker0P kerf0).
  by rewrite f1K idKf.
...
Proof.
move=> /kHom'P[idKf fM] /kHom'P[idKg gM]; apply/kHom'P; split=> [x Kx | x y Ex Ey].
...
by have [idKf _] := kHom'P homEf; rewrite idKf ?mem1v ?oner_eq0.
Qed.
...
pose f1 := (inLz^-1 \o f \o inLz)%VF; have /kHom'P[fFid fM] := homLf.
...
have [[g Dy] [idKy _]] := (kHom_to_AEnd homKy, kHom'P homKy).
...
have [[g Df] [idKf _]] := (kHom_to_AEnd KhomMf, kHom'P KhomMf).
...
have /kAut_to_gal[x galEx -> //]: kAut K E f.
  ...
  by move: autKf; rewrite inE kAutfE => /kHom'P[].
