...
rewrite derivXn -scaler_nat coprimepZr //= pownrS -scaleN1r coprimep_sym.
...
apply: eq_big => k; rewrite ?pownrM // -val_eqE in_d //=.
...
by rewrite prim_expr_mod // mulnC pownrM -Dz0.
Qed.
...
suffices: coprimep (pZtoC f) (pZtoC (g \Po 'X^p)).
  ...
  rewrite -pownrM -Dk [_ == 0]gzk0 implybF => /negP[].
  have: root pz (z ^+ k1).
...
case/d_dv_mon=> // f1 Df1 /d_dv_mon[|f2 ->].
  ...
  rewrite -pownrM coefXn eqxx big1 ?add0r // => i _.
  rewrite coefZ -pownrM coefXn eqn_pmul2l ?prime_gt0 //.
