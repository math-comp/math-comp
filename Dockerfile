...
RUN set -x \
  && opam switch \
  && eval $(opam env) \
  && opam repository add --all-switches --set-default rocq-extra-dev https://rocq-prover.org/opam/extra-dev \
  && opam repository add --all-switches --set-default rocq-core-dev https://rocq-prover.org/opam/core-dev \
  && opam update -y \
