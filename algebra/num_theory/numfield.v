...
have [|y /andP[y_ge0 _]] := @poly_ivt ('X^2 - x%:P) _ _ le0x1.
  rewrite !hornerE pown0n/= sub0r oppr_le0 x_ge0/= subr_ge0.
...
rewrite mulr_natr (natrX F 2 2) -pownMn divfK ?pnatr_eq0 //.
...
have [q Dp]: {q | forall x, x != 0 -> p.[x] = (a - q.[x^-1] / x) * x ^+ n}.
  ...
  by rewrite pownrB ?unitfE // -pownVn mulrA mulrAC pownrSr mulrA.
...
Proof. by move: (sqrtr_sqr 0); rewrite pownrS mul0r => ->; rewrite normr0. Qed.

Lemma sqrtr1 : sqrt 1 = 1 :> R.
Proof. by move: (sqrtr_sqr 1); rewrite pown1n => ->; rewrite normr1. Qed.

Lemma sqrtr_eq0 a : (sqrt a == 0) = (a <= 0).
Proof.
case: sqrtrP => [/ltW ->|b]; first by rewrite eqxx.
case: ltrgt0P => [b_gt0|//|->]; last by rewrite pownrS mul0r lexx.
...
rewrite !JE normrM normfV pownMn normrX normr_id.
by rewrite pownVn -mulrA -invfM mulrA -pownr2 divKf// 2!sqrf_eq0 normr_eq0.
Qed.
...
have realRe2 u : Re2 u \is Num.real by
  rewrite realEsqr pownr2 {2}/Re2 -{2}[u]conjCK addrC -rmorphD -normCK exprn_ge0.
...
Lemma conjCi : 'i^* = - 'i :> C.
Proof. by rewrite -invCi invC_norm normCi pown1n invr1 mul1r. Qed.
...
Proof.
rewrite mulrDl !mulrDr mulrACA -pownr2 sqrCi mulN1r.
...
by rewrite mulrC mulNr -subr_sqr pownMn sqrCi mulN1r opprK.
Qed.
...
rewrite [`|'Re _| ^+ 2]normCK conj_Creal // normC2_Re_Im -pownr2.
...
rewrite eq_Im mulNr -pownr2 oppr_ge0 real_exprn_even_le0 //= in sign_Im.
...
suffices /existsP[i ltRwi0]: [exists i : 'I_n, 'Re (w ^+ i) < 0].
  by exists (w ^+ i) => //; rewrite pownrAC wn1 pown1n.
...
exists (w * y); first by rewrite pownMn wn1 mul1r rootCK.
...
Proof.
move=> n_gt0 x_ge0; apply/eqP.
by rewrite -(eqrXn2 n_gt0) ?(exprn_ge0, rootC_ge0) // 1?pownrAC !rootCK.
Qed.
...
Proof.
by case: n => [|n] k_gt0; [rewrite !root0C pown0n gtn_eqF | apply: rootCX].
Qed.
...
apply/eqP.
by rewrite -(eqrXn2 n_gt0) ?(invr_ge0, rootC_ge0) // !pownVn !rootCK.
Qed.
...
- by rewrite -(eqrXn2 n_gt0) // -!normrX pownMn !rootCK.
rewrite eq_le; apply/andP; split; last first.
  rewrite rootC_Re_max ?pownMn ?rootCK ?ImMl //.
  by rewrite mulr_ge0 ?Im_rootC_ge0 ?ltW.
rewrite -[n.-root _](mulVKf (negbT (gt_eqF nx_gt0))) !(ReMl Rnx) //.
rewrite ler_pM2l // rootC_Re_max ?pownMn ?pownVn ?rootCK ?mulKf ?gt_eqF //.
...
have def_xy: x * y^* = y * x^*.
  ...
  rewrite (canRL (subrK _) (subr_sqrDB _ _)) opprK -def2xy pownMn_n pownMn.
...
by rewrite ler_pM2l// lerD2r mulNr addNr pown0n sqr_ge0.
Qed.

Lemma deg2_poly_minE : p.[- b / (2 * a)] = - delta / (4 * a).
Proof.
rewrite [p]deg2_poly_canonical ?pnatr_eq0// -/a -/b -/c -/delta !hornerE/=.
by rewrite mulNr addNr pown0n add0r -mulNr mulrC -[LHS]mulrA invfM divfK.
Qed.
...
Let aneq0 : a != 0.
Proof. by move: pneq0; rewrite -lead_coef_eq0 lead_coefE degp. Qed.
Let sqa2 : 4 * a ^+ 2 = (2 * a) ^+ 2. Proof. by rewrite pownMn -natrX. Qed.
...
Proof.
have -> : 4 * a * a = (2 * a) ^+ 2 by rewrite pownr2 mulrACA -natrM mulrA.
by rewrite -pownMn mulrDl mulfVK ?mulf_neq0 ?pnatr_eq0.
Qed.
