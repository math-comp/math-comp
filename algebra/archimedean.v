...
Lemma ge_trunc x : (trunc x)%:R <= x = (0 <= x).
Proof. by have := (Def.truncP x); case: ifP => [+ /andP[] | + /eqP->//]. Qed.

Lemma real_lt_succ_trunc x : x \is Rreal -> x < (trunc x).+1%:R.
Proof. by move/real_ge0P => [/trunc_itv/andP[]|/lt_le_trans->]. Qed.

Lemma trunc_ge_nat x n : 0 <= x -> (n <= trunc x)%N = (n%:R <= x).
Proof.
...
Lemma trunc_gt_nat x n : (n < trunc x)%N = (n.+1%:R <= x).
Proof.
have := Def.truncP x; case: ifP => [x0 _ | x0 /eqP->].
  by rewrite trunc_ge_nat.
by rewrite ltn0; apply/esym/(contraFF _ x0)/le_trans.
Qed.

Lemma trunc_lt_nat x n : 0 <= x -> (trunc x < n)%N = (x < n%:R).
Proof. by move=> ?; rewrite real_ltNge ?ger0_real// ltnNge trunc_ge_nat. Qed.

Lemma real_trunc_le_nat x n : x \is real -> (trunc x <= n)%N = (x < n.+1%:R).
Proof. by move=> ?; rewrite real_ltNge// leqNgt trunc_gt_nat. Qed.

Lemma trunc_eq x n : 0 <= x -> trunc x == n = (n%:R <= x < n.+1%:R).
Proof.
by move=> xr; apply/eqP/idP => [<-|]; [exact: trunc_itv|exact: trunc_def].
Qed.

Lemma trunc_le : {homo trunc : x y / x <= y >-> (x <= y)%N}.
Proof.
move=> x y lexy; move: (Def.truncP x) (Def.truncP y).
...
Lemma natrKtrunc : cancel (GRing.natmul 1) trunc.
Proof. by move=> n; apply: trunc_def; rewrite lexx -natr1 ltrDl ltr01. Qed.

Lemma natrEtrunc x : x \is a nat_num = ((trunc x)%:R == x).
Proof.
by apply/natrP/eqP => [[n ->] | <-]; [rewrite natrKtrunc | exists (trunc x)].
Qed.
...
Proof.
have := (Def.truncP x); case: ifP => [x0 | x0 /eqP<-].
  by rewrite trunc_ge_nat.
...
Lemma real_abs_floor_ge x : x \is Rreal -> `|x| <= `|floor x|.+1%:R.
Proof.
...
Lemma real_abs_ceil_ge x : x \is Rreal -> `|x| <= `|ceil x|.+1%:R.
...
Lemma lt_succ_trunc x : x < (trunc x).+1%:R.
Proof. exact: real_lt_succ_trunc. Qed.

Lemma trunc_le_nat x n : (trunc x <= n)%N = (x < n.+1%:R).
...
Lemma abs_floor_ge x : `|x| <= `|floor x|.+1%:R.
