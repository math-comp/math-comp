...
(*                                                                            *)
(* See ring.v for more documentation                                          *)
(* and test-suite/test_lra.v for examples of use.                             *)
(*                                                                            *)
(*  * Preprocessing additive and ring morphisms                               *)
(*                                                                            *)
(* See ring_tactic.v corresponding header section, with following addition:   *)
(* - `GRing.inv` (`_^-1`, pushed down)                                        *)
(******************************************************************************)
...
rewrite IH; first by rewrite orclP clacc -!orb_andr orbA orbb.
...
#[local] Notation Feval := (Feval 0 1 +%R *%R (fun x y : R => x - y) -%R
  N.to_nat (@GRing.exp R) isBool eq_op (cneqb eq_op) <=%R <%R R_of_C
  (env_nth 0)).
...
Section BFKFeval.
Variables (R : Type) (rO rI : R) (radd rmul : R -> R -> R) (ropp : R -> R).
Variable (rpow : R -> N -> R).
Variable req rneq rle rlt : R -> R -> bool.
Variable (C : Type) (R_of_C : C -> R).

Definition KFeval k : seq R -> Formula C -> eKind k :=
  let ev k eq neq le lt := Feval rO rI radd rmul (fun x y => radd x (ropp y))
    ropp id rpow k eq neq le lt R_of_C (env_nth rO) in
...
Definition RFeval (R : numDomainType) C (R_of_C : C -> R) := KFeval 0 1
  +%R *%R -%R (fun x n => x ^+ N.to_nat n) eq_op (cneqb eq_op) <=%R <%R R_of_C.

Definition RBFeval (R : numDomainType) C (R_of_C : C -> R) := BFKFeval 0 1
  +%R *%R -%R (fun x n => x ^+ N.to_nat n) eq_op (cneqb eq_op) <=%R <%R R_of_C.
...
- case=> f u; last by apply/eqP; rewrite -RBFeval_map_AC_of_C_bool.
  ...
  pose Feval_R := bool_R_eq (Feval_R erefl erefl erefl2 erefl2 erefl2 erefl1
    rN erefl2 (k_R:=isBool_R) erefl2b erefl2b erefl2b erefl2b
    R_of_C_R_of_AC renv_nth rl rf).
...
#[local] Notation Rnorm_expr := (Rnorm R_of_N (R_of_N false N0)
  (R_of_N false (Npos xH)) add mul opp_intr exp inv true false).
...
Ltac mclra_witness n f := let w := fresh "__wit" in mp_wlra_Q w f.
Ltac mcnra_witness n f := let w := fresh "__wit" in mp_wnra_Q w f.
...
Elpi Accumulate File ring_tactic arithmetic_tactic.
