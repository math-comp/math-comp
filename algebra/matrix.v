...
Fact const_mx_is_semi_additive : semi_additive const_mx.
...
Fact swizzle_mx_is_semi_additive k : semi_additive (swizzle_mx k).
Proof. by split=> [|A B]; apply/matrixP => i j; rewrite !mxE. Qed.
...
Fact diag_mx_is_semi_additive n : semi_additive (@diag_mx n).
Proof.
...
Fact scalar_mx_is_semi_additive : semi_additive scalar_mx.
...
Fact mxtrace_is_semi_additive : semi_additive mxtrace.
Proof.
...
Definition mulmxr m n p (B : 'M[R]_(n, p)) (A : 'M[R]_(m, n)) := mulmx A B.
Arguments mulmxr {m n p} B A /.
...
Fact scalar_mx_is_multiplicative : multiplicative (@scalar_mx R n).
...
Definition lin_mulmxr B := lin_mx (@mulmxr m n p B).
...
Fact map_mx_is_multiplicative n : multiplicative (map_mx f : 'M_n -> 'M_n).
