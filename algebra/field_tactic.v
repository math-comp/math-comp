...
(*  field by <tac> == a tactic for field equalities, uses the tactic          *)
(*                    `by <tac>` to discharge each subgoal stating non nullity*)
(*                    of denominators                                         *)
(*           field := field by field_tactic                                   *)
(*                    `field_tactic` (`done` by default) can be customized    *)
(*                    with `Ltac field_tactic ::= ...`                        *)
(*          field? == nonterminating variant that let the user handle the     *)
(*                    subgoals. This is intended for development purpose only *)
(*                    and shouldn't remain in final proof scripts.            *)
(*  field[?]: e1 ... en [by <tac>] == same as field, assuming ring equalities *)
(*                    e1,..., en                                              *)
(* These tactics work on any fieldType and handle additive morphisms          *)
...
(* this feature is the same as the `ring` tactic: `field: e1 .. en`.          *)
(*                                                                            *)
...
(* solved with the given tactic or `field_tactic`. During development process,*)
(* the `field?` tactic can be used to inspect these proof obligations.        *)
(*                                                                            *)
...
msolve GL SubGL :-
  coq.warning "mathcomp.dev-tactic" "mathcomp.dev-field"
    "`field?` is for development purposes, use `field` or `field by` in final scripts.",
...
Tactic Notation "field" "by" tactic(T) :=
  elpi mcfield; field_term T.
Tactic Notation "field" ":" ne_constr_list(L) "by" tactic(T) :=
  elpi mcfield ltac_term_list:(L); field_term T.
Tactic Notation "#[" attributes(A) "]" "field" "by" tactic(T) :=
  ltac_attributes:(A) elpi mcfield; field_term T.
Tactic Notation "#[" attributes(A) "]" "field" ":" ne_constr_list(L)
    "by" tactic(T) :=
  ltac_attributes:(A) elpi mcfield ltac_term_list:(L); field_term T.

Ltac field_tactic := done.

Tactic Notation "field" :=
  elpi mcfield; field_term field_tactic.
Tactic Notation "field" ":" ne_constr_list(L) :=
  elpi mcfield ltac_term_list:(L); field_term field_tactic.
Tactic Notation "#[" attributes(A) "]" "field" :=
  ltac_attributes:(A) elpi mcfield; field_term field_tactic.
Tactic Notation "#[" attributes(A) "]" "field" ":" ne_constr_list(L) :=
  ltac_attributes:(A) elpi mcfield ltac_term_list:(L); field_term field_tactic.
