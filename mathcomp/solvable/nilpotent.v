...
pose gTn := {dffun forall i, gT i}.
pose gTn' := {dffun forall i, gT' i}.
pose f (x : gTn) : gTn' := [ffun i => x (lift ord0 i)].
...
have km : morphic (G ord0) (@dfung1 _ _ ord0).
  by apply/morphicP => ? *; apply: dfung1_morphM.
...
apply/setP => x; rewrite morphimEdom; apply/idP/imsetP => [xker|].
  ...
  rewrite /= morphmE; apply/ffunP => i; rewrite ffunE.
  case: (unliftP ord0 i) => [/= j|] ->; last by rewrite dfwithin.
  rewrite dfwithout//.
  by have /mker/= := xker; rewrite morphmE => /ffunP/(_ j); rewrite !ffunE.
move=> [x0 xG0 -> /=]; apply/kerP; rewrite ?morphmE ?inE/= ?morphmE.
  by apply/forallP => i /=; rewrite ffunE/=; case: dfwithP.
by apply/ffunP=> i; rewrite !ffunE dfwithout.
Qed.
