...
have [ms0 os0]: m s0 = (p ^ n).+1%:R /\ #[s0] = p.
  ...
  rewrite subnK // mulSn -mulnA -expnS -addSn natrD natrM -oG char'_Zp //.
  rewrite mulr0 addr0 => m_s0; split => //.
  have [d _] := m_se (n - e0)%N; rewrite -subnSK // expnSr expgM -/s0.
  rewrite addSn subnK // -oG  mulrS natrM char'_Zp // {d}mulr0 addr0.
...
rewrite G4; exists s; split=> //; last first.
  ...
  by rewrite prednK ?expn_gt0 // addnn -mul2n -expnS -p2 -oG char'_Zp.
suffices TIst: <[s]> :&: <[t]> = 1.
