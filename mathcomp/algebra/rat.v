...
Definition fracq_subdef '((n', d')) : int * int :=
  match d', n' with
  | Posz 0, _ => (0, 1)
  | Posz 1, _ as n => (n, 1)
  | _ as d, _ as n => (fracq1_subdef n d, fracq2_subdef n d)
  end.
Arguments fracq_subdef : simpl never.

Fact fracqE_subdef x : fracq_subdef x =
  if x.2 != 0 then (fracq1_subdef x.1 x.2, fracq2_subdef x.1 x.2) else (0, 1).
Proof.
by case: x => [[n|n] [[|[|d]]|d]]//=; rewrite gcdn1 !divn1 ?mul1r ?mulN1r.
Qed.

Fact fracq_subproof x (y := fracq_subdef x) :
  (0 < y.2) && (coprime `|y.1| `|y.2|).
Proof.
rewrite {}/y fracqE_subdef/=; case: x => [/= n d]; apply/andP. split.
  have /implyP : (`|d| > 0) -> (gcdn `|n| `|d| <= `|d|)%N.
    by move=> d_neq0; rewrite dvdn_leq ?dvdn_gcdr.
  by case/intP: d => //= d; rewrite ltz_nat divn_gt0 ?gcdn_gt0 ?orbT.
have [|d_neq0]//= := eqVneq d 0; rewrite abszM/= abszX abszN1 exp1n mul1n.
rewrite /coprime -(@eqn_pmul2r (gcdn `|n| `|d|)); last first.
  by rewrite gcdn_gt0 ?absz_gt0 d_neq0 orbT.
by rewrite muln_gcdl !divnK ?mul1n ?(dvdn_gcdl, dvdn_gcdr) ?gcdn_gt0.
Qed.

Definition fracq '((n', d')) : rat :=
  match d', n' with
  | Posz 0 as d, _ as n | Posz 1 as d, _ as n
  | _ as d, Posz _ as n | _ as d, _ as n   => Rat (fracq_subproof (n, d))
  end.
Arguments fracq : simpl never.

Lemma val_fracq x : val (fracq x) =
  if x.2 != 0 then (fracq1_subdef x.1 x.2, fracq2_subdef x.1 x.2) else (0, 1).
Proof. by case: x => [[n|n] [[|[|d]]|d]]//=; rewrite fracqE_subdef. Qed.
...
Definition addq '(Rat _ xP as x) '(Rat _ yP as y) :=
   fracq (addq_subdef (valq x) (valq y)).
Lemma addqE x y : addq x y = fracq (addq_subdef (valq x) (valq y)).
Proof. by case: x; case: y. Qed.

Definition oppq_subdef (x : int * int) := (- x.1, x.2).
Definition oppq '(Rat _ xP as x) := fracq (oppq_subdef (valq x)).
Definition oppqE x : oppq x = fracq (oppq_subdef (valq x)).
Proof. by case: x. Qed.
...
rewrite /oppq_subdef; case: fracqP => /= [|u fx u_neq0].
  by rewrite fracq0.
by rewrite oppqE -mulrN fracqMM.
Qed.
...
Proof. by move=> x y; rewrite !addqE /= addq_subdefC. Qed.
...
Definition mulq '(Rat _ xP as x) '(Rat _ yP as y) :=
  fracq (mulq_subdef (valq x) (valq y)).

Lemma mulqE x y : mulq x y = fracq (mulq_subdef (valq x) (valq y)).
Proof. by case: x; case: y. Qed.
...
Definition invq '(Rat _ xP as x) := fracq (invq_subdef (valq x)).

Lemma invqE x : invq x = fracq (invq_subdef (valq x)).
Proof. by case: x. Qed.

Fact mulq_frac x y : (mulq (fracq x) (fracq y)) = fracq (mulq_subdef x y).
Proof.
rewrite mulqE /mulq_subdef; case: (fracqP x) => /= [|u fx u_neq0].
...
Proof.
by rewrite invqE /invq_subdef; case: (fracqP x) => // k {}x k0; rewrite fracqMM.
Qed.

Fact mulqC : commutative mulq.
Proof. by move=> x y; rewrite !mulqE mulq_subdefC. Qed.
...
rewrite [_ + _]addqE /numq /= -!/(denq _) ?mulf_eq0 ?denq_eq0.
...
have hxy: (0 <= numq x * denq y + numq y * denq x).
  by rewrite addr_ge0 ?mulr_ge0.
rewrite [_ * _]mulqE /numq /= -!/(denq _) ?mulf_eq0 ?denq_eq0.
rewrite val_fracq/=; case: ifP => //=.
by rewrite !mulr_ge0// !le_gtF ?mulr_ge0 ?denq_ge0//=.
Qed.
