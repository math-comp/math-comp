...
Reserved Notation "A ^C"    (at level 1, format "A ^C").

Notation "''A_' ( m , n )" := 'M_(m, n ^ 2)
  (at level 0, format "''A_' ( m ,  n )") : type_scope.

Notation "''A_' ( n )" := 'A_(n ^ 2, n)
  (at level 0, only parsing) : type_scope.

Notation "''A_' n" := 'A_(n)
  (at level 0, n at next level, format "''A_' n") : type_scope.
...
Notation "''A' [ F ]_ n" := 'A[F]_(n)
  (at level 0, n at level 2, only parsing) : type_scope.
...
Arguments genmx {F} {n m}%N A%MS : rename.
