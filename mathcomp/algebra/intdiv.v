...
Lemma dvdz_char'f (R : ringType) p : p \in [char' R] ->
  forall n : int, (p %| n)%Z = (n%:~R == 0 :> R).
Proof.
move=> char'Rp [] n; rewrite [LHS](dvdn_char'f char'Rp)//.
by rewrite NegzE abszN rmorphN// oppr_eq0.
Qed.

#[deprecated(since="mathcomp 2.4.0", note="Use dvdz_char'f  instead.")]
Notation dvdz_charf chRp := (dvdz_char'f chRp).
...
have c_neq0 : (lead_coef q)%:~R != 0 :> 'F_p
   by rewrite -(dvdz_char'f (char'_Fp _)).
have : map_poly (intr : int -> 'F_p) q = (lead_coef q)%:~R *: 'X^(size q).-1.
  ...
  by apply/eqP; rewrite -(dvdz_char'f (char'_Fp _))// dvd_pq.
...
by rewrite !coef_map// -!(dvdz_char'f (char'_Fp _))//; apply: dvdz_mul.
Qed.
