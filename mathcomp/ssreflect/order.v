...
Definition converse_latticeMixin :=
   @LatticeMixin _ [porderType of L^c] _ _ joinC meetC
                 joinA meetA meetKU joinKI converse_leEmeet.
...
Definition converse_distrLatticeMixin :=
   @DistrLatticeMixin _ [latticeType of L^c] joinIl.
...
Definition converse_tbLatticeMixin :=
  @TBLatticeMixin _ [bLatticeType of L^c] (bottom : L) (@le0x _ L).
...
Canonical converse_bDistrLatticeType := [bDistrLatticeType of L^c].
Canonical converse_tbDistrLatticeType := [tbDistrLatticeType of L^c].
...
Proof.
move=> x y; rewrite /<=%O /= !le_def (eq_sym y).
