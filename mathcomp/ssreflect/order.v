...
(*                                                                            *)
(* * Displays                                                                 *)
(* Each generic interface for ordered types has, as its first argument, a     *)
(* "display" of type Order.disp_t to control the printing of notations. For   *)
(* example, when m and n are of type natdvd (an alias of nat), (Order.le m n) *)
(* is displayed m %| n; natdvd being associated to the display dvd_display.   *)
(*                                                                            *)
(* We stress that a display can only control printing, but not parsing. For   *)
(* example, writing m %| n does not let you trigger an inference of the       *)
(* instance associated to natdvd; such an inference should be triggered       *)
(* through the use of a type alias, e.g., (m : natdvd) %| n.                  *)
(* In other words, beware of the asymmetry of canonical structure inference!  *)
(* Canonical structure inference makes it possible to infer the display       *)
(* associated to a type (or a named copy of a type) from the name. However,   *)
(* it does not help finding the named copy when the display is provided. In   *)
(* this sense displays are merely used to inform the user and the notation    *)
(* mechanism of what the inference did; they are not additional input for the *)
(* inference.                                                                 *)
(*                                                                            *)
(* See below for various "copies" of types and their associated displays.     *)
(*                                                                            *)
...
(*                              operation for each interval of the form [0, b]*)
...
(*                              operation for each interval of the form [a, 1]*)
...
(* default notation (defined in order_scope unless noted otherwise).          *)
(*                                                                            *)
...
(*                                                                            *)
(* We provide the following "copies" of types and their displays:             *)
