...
(* "displays". Each generic interface for ordered types has, as its first     *)
(* argument, a display of type Order.disp_t to control the printing of        *)
(* notations. For example, when m and n are of type natdvd (an alias of nat), *)
(* (Order.le m n) is displayed m %| n; natdvd being associated to the display *)
(* dvd_display. We stress that a display can only control printing, but not   *)
(* parsing. For example, writing m %| n does not let you trigger an inference *)
(* of the instance associated to natdvd; such an inference should be          *)
(* triggered through the use of a type alias, e.g., (m : natdvd) %| n.        *)
(* Instantiating d with Disp tt tt or an unknown display will lead to a       *)
(* default display for notations. See "Order relations and operations" below  *)
