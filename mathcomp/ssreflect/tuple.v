...
From mathcomp Require Import seq choice div fintype.
...
rewrite addn1 (addnC 0 i.+1) iota_addl -map_comp !/comp add0n addn0.
have ->:
  sumn [seq f (i.+1 + j) * $|T| ^ (i.+1 + j) | j <- iota 0 (n - i.+1)] =
  $|T| ^ (i.+1) * sumn [seq f (i.+1 + j) * $|T| ^ j | j <- iota 0 (n - i.+1)].
  by elim: iota => /= [|j js ->]; rewrite ?muln0 // expnD mulnCA mulnDr.
rewrite expnS -mulnAC -addnA -mulnDl addnC divnMDl; last first.
  by rewrite raw_cardE expn_gt0 HT.
