...
Fixpoint Bezout_rec km kn qs :=
  if qs is q :: qs' then Bezout_rec kn ((q * kn) + km) qs'
  else (km, kn).
...
case: posnP => [r0 {s le_ns1 IHs lt_rn}|r_gt0]; last first.
  by apply: IHs => //=; [rewrite -def_m | rewrite (leq_trans lt_rn)].
...
rewrite [bz in Spec bz](_ : _ = Bezout_rec km kn qs); last first.
  by rewrite /kn /km; case: (b) => //=; rewrite addn0 muln1.
...
set m := _ + r; set km := _ + kn; pose d := gcdn m n.
