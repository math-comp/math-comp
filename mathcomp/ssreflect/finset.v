...
have := H2 x ; have := H1 x.
rewrite /= Hx !andbT => Hx1 /negP/negP Hx2 ; by rewrite -in_setC setCK Hx2 in Hx1.
Qed.

Lemma subset_disjoint A B : [disjoint A & B] -> forall C, C != set0 -> C \subset A -> ~~ (C \subset B).
Proof.
move => HAB C HC /subsetP /= H ; apply /subsetPn.
case (pickP [pred x in C]) => /= [x Hx|Hcontra].
- exists x => //= ; by rewrite (disjointFr HAB (H x Hx)).
- move/set0Pn in HC ; destruct HC as [x Hx] ; have := (Hcontra x) => //= ; by rewrite Hx.
Qed.
...
Proof. case: pickP=>[x|/(_ x0)] ; rewrite !inE ; try move=>/eqP->// ; by rewrite eqxx. Qed.
...
- destruct (eq_bigmax_cond (fun=>0%N) H0) as [w Hw _] ; destruct (cards1P H1) as [w' Hw'].
...
Variables (op : Monoid.law idx) (aop : Monoid.com_law idx) (times : Monoid.mul_law idx) (plus : Monoid.add_law idx times).
...
Lemma big_card1 (f : {set I} -> R) :
  \big[aop/idx]_(A : {set I} | #|A| == 1%N) f A = \big[aop/idx]_(i : I) f [set i].
Proof.
...
- rewrite /omap /obind /oapp /set1_oinv.
  ...
  move: HA => /cards1P [j Hj] ; rewrite Hj in_set1 in Hi ; by rewrite Hj (eqP Hi).
Qed.

Lemma big_setI_distrl (P : pred {set I}) (h : {set I} -> {set I}) (f g : {set I} -> R) :
  \big[plus/idx]_(A : {set I} | P (h A)) times (g A) (f (h A))
  =
  \big[plus/idx]_(B : {set I} | P B) times (\big[plus/idx]_(A : {set I} | h A == B) g A) (f B).
Proof.
under [RHS]eq_bigr do rewrite big_distrl /=.
rewrite [LHS](partition_big h P) => //.
apply: eq_bigr => B HB ; apply: eq_big => [A | A /andP [_ HA2]].
- case (boolP (h A == B)) => H ; last by rewrite (negbTE H) andbF. by rewrite H (eqP H) HB. 
- by rewrite (eqP HA2).
Qed.

Lemma big_partitionS (f : {set I} -> I -> R) :
  \big[aop/idx]_(A : {set I}) (\big[aop/idx]_(i in A) f A i) = \big[aop/idx]_(i : I) \big[aop/idx]_(A : {set I} | i \in A) f A i.
Proof.
...
- have proof1 :
  ...
  apply eq_bigr => i _.
  set f1inv : {set I} -> option pair_sig := fun A => match (boolP (i \in A)) with AltTrue h => Some (exist _ (A,i) h) | _ => None end.
  ...
  + apply eq_big => /= s ;  destruct s as [[A j] Hj] ; simpl in Hj.
    * rewrite /f1/f1inv/f2 => /= ; case (boolP (i \in A)) => Hi /=.
      - case (boolP (j == i)) => H0 //=.
        + apply/eqP ; apply f_equal.
          have H :  (@sval) ({set I} * I) (fun a : {set I} * I => a.2 \in a.1) (exist (fun i0 : {set I} * I => i0.2 \in i0.1) (A, i) Hi) =
                    (@sval) ({set I} * I) (fun a : {set I} * I => a.2 \in a.1) (exist (fun p : {set I} * I => p.2 \in p.1) (A, j) Hj).
          by simpl ; rewrite (eqP H0).
          apply (eq_sig _ _ H) => //= ; exact: (Eqdep_dec.eq_proofs_unicity (@eq_comparable_prop _)).
    ...
    * rewrite /f1/f2 => /= /andP [Hi Hinv].
      move: Hinv ; rewrite /f1inv ; case (boolP (i\in A)) => H ; last by rewrite Hi in H.
      by move/eqP ; case => ->.
  + rewrite /omap/obind/oapp/f1inv => A Hi ; by case (boolP (i \in A)) => // ; rewrite Hi.
- have proof2 :
    \big[aop/idx]_(A : {set I}) (\big[aop/idx]_(i in A) f A i) =
    \big[aop/idx]_(A : {set I}) (\big[aop/idx]_(s : pair_sig | f1 s == A) f (f1 s) (f2 s)).
  apply eq_bigr => A _.
  set f2inv : I -> option pair_sig := fun i => match (boolP (i \in A)) with AltTrue h => Some (exist _ (A,i) h) | _ => None end.
  rewrite (reindex_omap f2 f2inv) => /=.
  + apply eq_big => /= s ;  destruct s as [[B i] HB] ; simpl in HB.
    * rewrite /f1/f2inv/f2 => /= ; case (boolP (i \in A)) => HA /=.
      - case (boolP (B == A)) => H0 //=.
        + apply/eqP ; apply f_equal.
          have H :  (@sval) ({set I} * I) (fun a : {set I} * I => a.2 \in a.1) (exist (fun p : {set I} * I => p.2 \in p.1) (A, i) HA) =
                    (@sval) ({set I} * I) (fun a : {set I} * I => a.2 \in a.1) (exist (fun p : {set I} * I => p.2 \in p.1) (B, i) HB).
          by simpl ; rewrite (eqP H0).
          apply (eq_sig _ _ H) => //= ; exact: (Eqdep_dec.eq_proofs_unicity (@eq_comparable_prop _)).
        + by apply/eqP ; case => /eqP ; rewrite eq_sym (negbTE H0).
      - symmetry ; apply/eqP => Hcontra ; by rewrite -Hcontra HB in HA.
    * rewrite /f1/f2 => /= /andP [Hi Hinv].
      move: Hinv ; rewrite /f2inv ; case (boolP (i\in A)) => H ; last by rewrite Hi in H.
      by move/eqP ; case => ->.
  + rewrite /omap/obind/oapp/f2inv => i Hi ; by case (boolP (i \in A)) => // ; rewrite Hi.
...
Lemma big_memS (f : {set I} -> R) (P : pred {set I}):
  \big[aop/idx]_(i : I) \big[aop/idx]_(A : {set I} | P A && (i \in A)) f A = \big[aop/idx]_(A | P A) \big[aop/idx]_(w in A) f A.
Proof.
