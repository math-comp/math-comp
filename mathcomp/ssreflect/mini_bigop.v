...
From mathcomp Require Import mini_ssreflect mini_ssrbool mini_ssrfun.
From mathcomp Require Import mini_eqtype mini_ssrnat mini_seq mini_path.
From mathcomp Require Import mini_div.
...
Lemma reindex_seq (I J : eqType) (h : J -> I) s r F :
   perm_eq r (map h s) ->
  \big[*%M/1]_(i <- r) F i = \big[*%M/1]_(j <- s) F (h j).
Proof. by move=> rs; rewrite (perm_big _ _ rs) big_map. Qed.
Arguments reindex_seq [I J] h s [r F].
...
pose r i := m + n - S i.
have rK: {in [pred k | S k <= m + (n - m)], involutive r}.
  move=> k k_lt_mn; suff Skmn : S k <= m + n.
    by rewrite /r subnSK ?subKn// ltnW.
  by move: k_lt_mn => /leq_trans->//; rewrite leq_add2l leq_subr.
rewrite (reindex_seq r (index_iota m n))//.
apply: uniq_perm; rewrite /index_iota ?map_inj_in_uniq ?iota_uniq//.
  by apply: can_in_inj (r) _ => k; rewrite mem_iota => /andP[_ kmn]; rewrite rK.
(* have mr k : (m <= r k) = (S k <= m + (n - m)). *)
(*   rewrite /r -leq_subLR. *)
(* move=> k; rewrite mem_iota; apply/idP/mapP. *)
(*   move=> /andP[km kmn]; exists (r k); last by rewrite rK// inE mnW. *)
(*   by rewrite mem_iota mr kmn/= -mr rK//. *)
(* by move=> [l]; rewrite mem_iota => /andP[lm lmn ->{k}]; rewrite mr lmn -mr rK. *)
Admitted.    
